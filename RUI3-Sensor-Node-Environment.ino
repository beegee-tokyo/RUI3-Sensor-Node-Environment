...
// Set region
// #if RUI_DEV == 1
	MYLOG("SET", "Set Class A %s", api.lorawan.deviceClass.set(0) ? "Success" : "Fail");
// #else
// 	MYLOG("SET", "Set Class C %s", api.lorawan.deviceClass.set(2) ? "Success" : "Fail");
// #endif
	...
	{
		MYLOG("SET", "Region %s", api.lorawan.band.set(g_lorawan_settings.lora_region) ? "Success" : "Fail");
	}

	MYLOG("SET", "TXP %s", api.lorawan.txp.set(g_lorawan_settings.tx_power) ? "Success" : "Fail");
		...
		(g_lorawan_settings.lora_region == RAK_REGION_CN470))
	{
		...
		MYLOG("SET", "Channel mask %s", api.lorawan.mask.set(&maskBuff) ? "Success" : "Fail");
		// maskBuff = 0x0000;
	...
	MYLOG("SET", "Join mode %s", api.lorawan.njm.set(g_lorawan_settings.otaa_enabled) ? "Success" : "Fail");

	// Set packet mode (confirmed/unconfirmed)
	if (g_lorawan_settings.confirmed_msg_enabled)
	{
		MYLOG("SET", "Conf. packets  %s", api.lorawan.cfm.set(0) ? "Success" : "Fail");
	}
	else
	{
		MYLOG("SET", "Unconf. packets  %s", api.lorawan.cfm.set(1) ? "Success" : "Fail");
	}
	...
	MYLOG("SET", "Send frequency  %s", init_frequency_at() ? "Success" : "Fail");
...
void sensor_handler(void *)
{
	...
	MYLOG("UPL", "Battery %.4f", api.system.bat.get());
